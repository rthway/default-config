...
FROM
    ...
    WHERE
    icd11_code IN ("5A01","5A14","5B7Z","5B7Y","5B81","3A9Z","8C0Z")) first_answers 
        LEFT OUTER JOIN
    (SELECT DISTINCT
        (p.person_id),
    ...
    INNER JOIN concept_name cn ON o.concept_id = cn.concept_id
     ...
     JOIN diagnosis_concept_view_new dcv ON dcv.concept_id = o.value_coded
        AND icd11_code IN ("5A01","5A14","5B7Z","5B7Y","5B81","3A9Z","8C0Z")
    WHERE
...
ORDER BY FIELD(first_answers.icd11_code,"5A01","5A14","5B7Z","5B7Y","5B81","3A9Z","8C0Z");
