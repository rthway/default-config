SELECT 
    first_answers.icd11_code AS 'ICD11',
    first_answers.answer_name AS 'Disease Name',
    
IFNULL(SUM(CASE WHEN second_concept.gender = 'F' AND second_concept.person_id IS NOT NULL AND second_concept.age_grp = '≤ 7 Days'  THEN 1 ELSE 0  END), 0) AS 'F ≤ 7 Days',
IFNULL(SUM(CASE WHEN second_concept.gender = 'M' AND second_concept.person_id IS NOT NULL AND second_concept.age_grp = '≤ 7 Days'  THEN 1 ELSE 0  END), 0) AS 'M ≤ 7 Days',

IFNULL(SUM(CASE WHEN second_concept.gender = 'F' AND second_concept.person_id IS NOT NULL AND second_concept.age_grp = '8 - 28 Days'  THEN 1 ELSE 0  END), 0) AS 'F 8 - 28 Days',
IFNULL(SUM(CASE WHEN second_concept.gender = 'M' AND second_concept.person_id IS NOT NULL AND second_concept.age_grp = '8 - 28 Days'  THEN 1 ELSE 0  END), 0) AS 'M 8 - 28 Days',
...
IFNULL(SUM(CASE WHEN second_concept.gender = 'F' AND second_concept.person_id IS NOT NULL AND second_concept.age_grp = '60 ‐ 69 Years'  THEN 1 ELSE 0  END), 0) AS 'F 60 ‐ 69 Years',
IFNULL(SUM(CASE WHEN second_concept.gender = 'M' AND second_concept.person_id IS NOT NULL AND second_concept.age_grp = '60 ‐ 69 Years'  THEN 1 ELSE 0  END), 0) AS 'M 60 ‐ 69 Years',

IFNULL(SUM(CASE WHEN second_concept.gender = 'F' AND second_concept.person_id IS NOT NULL AND second_concept.age_grp = '≥ 70 Years'  THEN 1 ELSE 0  END), 0) AS 'F ≥ 70 Years',
IFNULL(SUM(CASE WHEN second_concept.gender = 'M' AND second_concept.person_id IS NOT NULL AND second_concept.age_grp = '≥ 70 Years'  THEN 1 ELSE 0  END), 0) AS 'M ≥ 70 Years',
   combo_id as ICD_ID
FROM
    (SELECT 
   ...
   (SELECT DISTINCT
    ...
    JOIN dhis2_icd_map dhis_map on dcv.icd11_code = dhis_map.icd_code
    WHERE
		...
		(SELECT DISTINCT
			...
			INNER JOIN reporting_age_group_new rag ON visit.date_started BETWEEN (DATE_ADD(
...
GROUP BY first_answers.icd11_code, first_answers.answer_name
ORDER BY first_answers.icd11_code;
